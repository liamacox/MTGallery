...
public static class ScryfallApiClient
{
    ...
    public static async Task<List<Card>> GetSpecialGuestDataAsync(string[] ranges, string setCode)
    {
        List<Card> cards = [];
        ...
        var queryString = $"https://api.scryfall.com/cards/search?q=set%3Aspg+cn>={ranges[0]}+cn<={ranges[1]}";
        JsonDocument responseJson;
        do
        {
            Thread.Sleep(200);
